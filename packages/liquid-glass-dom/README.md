...
`liquid-glass-dom` provides the core DOM-backed liquid-glass renderer. It includes an imperative scene graph, a WebGPU renderer that owns a canvas, a reusable WebGPU core for adapters, and a retained layout API under `liquid-glass-dom/layout`.
...
`liquid-glass-dom/layout` wraps the imperative scene graph in retained layout nodes powered by `laymeout`.
...
pnpm --filter liquid-glass-dom watch
```
