...
`liquid-glass-react` provides React 19 bindings for the retained liquid-glass layout API. It lets React describe a retained scene while the renderer mutates layout nodes directly between renders.
...
These components mirror the retained layout classes from `liquid-glass-dom/layout`.
...
```tsx
import { Glass, GlassContainer, Html } from 'liquid-glass-react'
```
...
React components ultimately synchronize into the `liquid-glass-dom` scene graph, so nested children must satisfy the underlying parent rules:
...
- Rendering requires WebGPU through `liquid-glass-dom`.
...
```sh
pnpm --filter liquid-glass-react build
pnpm --filter liquid-glass-react test
pnpm --filter liquid-glass-react watch
```
